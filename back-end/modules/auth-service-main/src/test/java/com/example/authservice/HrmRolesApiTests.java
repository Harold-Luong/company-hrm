...
class HrmRolesApiTests {
    ...
    void serviceAlsoRejectsRegistrationWithoutHrOrAdmin(UserRole role) {
        ...
        try {
            assertThrows(AccessDeniedException.class, () -> authService.register(
                    new RegisterRequest("employee@example.com", "password123", Set.of(UserRole.EMPLOYEE), UUID.fromString("550e8400-e29b-41d4-a716-000000001001"))));
            assertEquals(1, users.count());
        } finally {
    ...
    void adminCreatesEachHrmRoleAndRoleFlowsThroughLoginRefreshAndMe(UserRole role) throws Exception {
        ...
        // A newly minted access token must reflect the current account role, not the
        // old access token.
    ...
    void omittedOrNullRolesDefaultToEmployee(String fields) throws Exception {
        var body = mapper.createObjectNode().put("employeeId", "550e8400-e29b-41d4-a716-000000001001").put("email", "employee@example.com")
    ...
    void rejectsEmptyOrMalformedRoleSets(String roles) throws Exception {
        mvc.perform(post("/api/v1/auth/register").header("Authorization", "Bearer " + adminToken)
                ...
                .content(
                        "{\"employeeId\":\"550e8400-e29b-41d4-a716-000000001001\",\"email\":\"employee@example.com\",\"password\":\"password123\",\"roles\":"
                                + roles + "}"))
                .andExpect(status().isBadRequest());
        assertEquals(1, users.count());
    }
    ...
    void multipleRolesPersistAndFlowThroughLoginMeAndRefresh() throws Exception {
        ...
        String rotated = mvc.perform(post("/api/v1/auth/refresh").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("refreshToken", refresh))))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        assertEquals(List.of("EMPLOYEE", "MANAGER"),
                jwtService.parseToken(mapper.readTree(rotated).path("accessToken").asText())
                        .get("roles", List.class));
    }
    ...
    private String registrationBody(String... roles) {
        return mapper.writeValueAsString(Map.of("email", "employee@example.com", "password", "password123", "roles",
                roles, "employeeId", "550e8400-e29b-41d4-a716-000000001001"));
    }
    ...
    void employeeLinkFlowsThroughRegistrationLoginMeAndRefresh() throws Exception {
        ...
        User user = users.findByEmail("employee@example.com").orElseThrow();
        assertEquals(UUID.fromString("550e8400-e29b-41d4-a716-000000001001"), user.getEmployeeId());
        String login = mvc.perform(post("/api/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", user.getEmail(), "password", "password123"))))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        String access = mapper.readTree(login).path("data").path("accessToken").asText();
        ...
        assertEquals("550e8400-e29b-41d4-a716-000000001001", jwtService.parseToken(access).get("employee_id", String.class));
        mvc.perform(get("/api/v1/auth/me").header("Authorization", "Bearer " + access))
                .andExpect(status().isOk()).andExpect(jsonPath("$.employeeId").value("550e8400-e29b-41d4-a716-000000001001"));

        user.setEmployeeId(UUID.fromString("550e8400-e29b-41d4-a716-000000001002"));
        users.saveAndFlush(user);
        mvc.perform(get("/api/v1/auth/me").header("Authorization", "Bearer " + access))
                .andExpect(status().isOk()).andExpect(jsonPath("$.employeeId").value("550e8400-e29b-41d4-a716-000000001002"));
        String rotated = mvc.perform(post("/api/v1/auth/refresh").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("refreshToken", refresh))))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        assertEquals("550e8400-e29b-41d4-a716-000000001002", jwtService.parseToken(mapper.readTree(rotated).path("accessToken").asText())
                .get("employee_id", String.class));
    }

    @ParameterizedTest
    @ValueSource(strings = { "null", "0", "-1", "1001", "\"1001\"", "\"\"", "\"invalid\"", "\"550e8400-e29b-41d4-a716-00000000100z\"", "{}" })
    ...
    void rejectsAnotherAccountForTheSameEmployee() throws Exception {
        var body = mapper.createObjectNode().put("email", "employee@example.com")
                .put("password", "password123").put("employeeId", admin.getEmployeeId().toString());
        mvc.perform(post("/api/v1/auth/register").header("Authorization", "Bearer " + adminToken)
